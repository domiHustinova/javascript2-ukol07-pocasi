...
/* harmony import */ var _weather_icons__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./weather-icons */ "./src/weather-icons.js");
...
var arrayDays = ['Neděle', 'Pondelí', 'Úterý', 'Středa', 'Čtvrtek', 'Pátek', 'Sobota'];
var forecastData = [];
...
var Forecast = /*#__PURE__*/function () {
  ...
  _createClass(Forecast, [{
    key: "getForecastData",
    value: function getForecastData(apiKey, apiUrl) {
      var _this = this;

      var params = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "q=Brno,cz";
      var query = fetch("".concat(apiUrl, "forecast?APPID=").concat(apiKey, "&").concat(params, "&units=metric&lang=CZ"));
      query.then(function (response) {
        return response.json();
      }).then(function (dataReceived) {
        forecastData = dataReceived.list.map(function (forecastDays) {
          return {
            temp: Math.round(forecastDays.main.temp),
            date: _this.getDate(forecastDays.dt),
            icon: Object(_weather_icons__WEBPACK_IMPORTED_MODULE_7__["default"])(forecastDays.weather[0].id, forecastDays.weather[0].icon)
          };
        }); // console.log(forecastData);

        _this.displayForecastData(forecastData);
      });
    }
  }, {
    key: "getDate",
    ...
    key: "displayForecastData",
    value: function displayForecastData(data) {
      var groups = data.reduce(function (groups, forecast) {
      ...
      var temps = Object.keys(groups).map(function (day) {
    ...
    key: "getHTML",
    value: function getHTML(list) {
      var _this2 = this;

      document.querySelector('#predpoved').innerHTML = list.reduce(function (total, curr) {
        return total + _this2.renderForecast(curr);
      }, '');
    }
  }, {
    key: "renderForecast",
    value: function renderForecast(object) {
      return "\n        <div class=\"forecast\">\n            <div class=\"forecast__day\">".concat(arrayDays[object.date.weekDay], " ").concat(object.date.day, ". ").concat(object.date.month, ".</div>\n            <div class=\"forecast__icon\">").concat(object.icon, "</div>\n            <div class=\"forecast__temp\">").concat(object.temp, " \xB0C</div>\n      </div>\n\n        ");
    }
  }]);
...
var apiKey = 'b873b3cd2a6fe46f2345df4722c65732';
var apiUrl = 'https://api.openweathermap.org/data/2.5/';
var weather = new _weather__WEBPACK_IMPORTED_MODULE_3__["default"]();
var forecast = new _forecast__WEBPACK_IMPORTED_MODULE_4__["default"]();
var location = new _geolocation__WEBPACK_IMPORTED_MODULE_5__["default"]();
weather.getWeatherData(apiKey, apiUrl);
forecast.getForecastData(apiKey, apiUrl);
document.querySelector("#currentLocation").addEventListener('click', /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
  var geo;
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 2:
          geo = _context.sent;
          console.log(geo);
          weather.getWeatherData(apiKey, apiUrl, "lat=".concat(geo.coords.latitude, "&lon=").concat(geo.coords.longitude));
          forecast.getForecastData(apiKey, apiUrl, "lat=".concat(geo.coords.latitude, "&lon=").concat(geo.coords.longitude));
...
document.querySelector("#praha").addEventListener('click', function () {
  weather.getWeatherData(apiKey, apiUrl, "q=Praha,cz");
  forecast.getForecastData(apiKey, apiUrl, "q=Praha,cz");
});
document.querySelector("#barcelona").addEventListener('click', function () {
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return getWeatherIcon; });
...
function getWeatherIcon(code, img) {
  ...
  return "<i class=\"".concat(prefix + icon, "\"></i>");
...
/* harmony import */ var _weather_icons__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./weather-icons */ "./src/weather-icons.js");
...
var Weather = /*#__PURE__*/function () {
  ...
  _createClass(Weather, [{
    key: "getWeatherData",
    value: function getWeatherData(apiKey, apiUrl) {
      var _this = this;

      var params = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "q=Brno,cz";
      var query = fetch("".concat(apiUrl, "weather?APPID=").concat(apiKey, "&").concat(params, "&units=metric&lang=CZ"));
      query.then(function (response) {
        return response.json();
      }).then(function (data) {
        return _this.displayWeatherData(data);
      });
    }
  }, {
    key: "getTime",
    ...
    key: "displayWeatherData",
    value: function displayWeatherData(data) {
      // console.log(data);
      var cityEl = document.querySelector("#mesto");
      var tempEl = document.querySelector("#teplota");
      var descriptionEl = document.querySelector("#popis");
      var iconEl = document.querySelector("#ikona");
      var humidityEl = document.querySelector("#vlhkost");
      var speedEl = document.querySelector("#vitr");
      var sunriseEl = document.querySelector("#vychod");
      var sunsetEl = document.querySelector("#zapad");
      cityEl.textContent = data.name;
      tempEl.textContent = Math.round(data.main.temp);
      descriptionEl.textContent = data.weather[0].description;
      iconEl.innerHTML = Object(_weather_icons__WEBPACK_IMPORTED_MODULE_2__["default"])(data.weather[0].id, data.weather[0].icon);
      humidityEl.textContent = data.main.humidity;
      speedEl.textContent = data.wind.speed.toFixed(1);
      var sunrise = this.getTime(data.sys.sunrise);
      var sunset = this.getTime(data.sys.sunset);
      sunriseEl.textContent = "".concat(sunrise.hours, ":").concat(sunrise.minutes);
      sunsetEl.textContent = "".concat(sunset.hours, ":").concat(sunset.minutes);
    }
  }]);
